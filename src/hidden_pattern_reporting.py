...
STRATEGY_NAMES_AR = {
    ...
    "fixed_tp_real_target": "هدف ربح ثابت عند الهدف الحقيقي (maxTP، أو 2R كحد أدنى لو غير معروف)",
...
def build_report(results: dict) -> str:
    ...
    lines.append("## ⚠️ تصحيح منهجي حول تعريف \"الهدف\" (بعد توضيح المستخدم)\n")
    lines.append(
        "نسخة سابقة من هذا التقرير استخدمت عمود `idealTP` مباشرة كـ\"الهدف\"، وكانت الأرقام الناتجة مبالَغ "
        "فيها بشكل كبير. **`idealTP` ليس هدفاً محدَّداً وقت الدخول — هو أعلى سعر وصلته الصفقة قبل ضرب وقف "
        "الخسارة الأصلي** (تحقَّقنا من هذا بمطابقته ضد حساب مستقل من الشموع نفسها؛ التفاصيل الكاملة في "
        "`src/idealtp_data_quality_check.py`). **الهدف الحقيقي** المُستخدَم بكل الجداول أدناه الآن هو: عمود "
        "`maxTP` لما يكون موجوداً (فقط للصفقات الـ213 التي أُغلقت برح حقيقي)، وإلا **ضعف مسافة وقف الخسارة "
        "(2R) كحد أدنى افتراضي** — حسب قاعدة المستخدم نفسه، لأن الهدف الحقيقي غير مسجَّل لصفقة لم تصله "
        "أصلاً. كل الأرقام بهذا التقرير مبنية على هذا التعريف المصحَّح.\n"
    )
    ...
    lines.append(
        ...
        f"مختلفة عن بعضها: نفس الاستراتيجية (\"{_ar('fixed_tp_real_target')}\") فازت في **كلا** العنقودين، وفي "
        ...
        f"- **الاكتشاف الأهم، بأرقام محافظة وصادقة بعد التصحيح**: تركُ الصفقة تجري حتى هدفها الحقيقي "
        f"(`maxTP` أو 2R كحد أدنى) بدلاً من الإغلاق اليدوي الفعلي رفع معامل الربح من "
        f"**{_pf(baseline_row['profit_factor'])}** إلى **{_pf(hybrid_row['profit_factor'])}** (تحسّن "
        f"{_fmt(hybrid_row['pf_improvement_pct'], 0)}%)، ورفع العائد المتوقع من {_fmt(baseline_row['expectancy'])}R "
        f"إلى {_fmt(hybrid_row['expectancy'])}R للصفقة، وخفّض أقصى تراجع من {_fmt(dd['baseline_max_dd_r'], 1)}R "
        f"إلى {_fmt(dd['hybrid_max_dd_r'], 1)}R. **هذا تحسّن حقيقي ومتواضع (12-13%)، وليس التحسّن الدراماتيكي "
        "(110%+) الذي ذُكر بنسخة سابقة خاطئة من هذا التقرير.**\n"
        "- **الربط بنتيجة \"441/789 صفقة خروج سيء\" من التحليل الأصلي يبقى مثيراً للاهتمام لكن أضعف مما ذُكر "
        "سابقاً**: جزء من هذا السوء قد يرتبط بعدم الالتزام بهدف حقيقي، لكن بحجم تأثير متواضع وليس دراماتيكياً.\n"
    )
    ...
    lines.append(
        ...
        "— النتيجة الإيجابية بأكملها تأتي من استراتيجية واحدة عامة (الهدف الحقيقي)، وليس من ذكاء \"هجين\" حقيقي.\n"
        "- **576 من 789 صفقة (73%) ما إلها هدف حقيقي مسجَّل** (`maxTP` فاضٍ لأنها لم تُغلَق برح) — استُخدم لها "
        "افتراض 2R كحد أدنى (حسب قاعدة المستخدم)، وهو تقدير وليس الهدف الحقيقي الذي كان مقصوداً وقت الدخول. "
        "أي جزء من النتيجة يعتمد على هذه الـ576 صفقة يحمل هذا التحفّظ الإضافي.\n"
        "- **الهدف الحقيقي قد لا يتحقق دائماً في التداول الحي بنفس الطريقة**: المحاكاة تفترض تنفيذ أمر معلَّق "
        "(limit order) يُملأ بالضبط عند مستوى الهدف فور لمسه، بدون انزلاق سعري (slippage) أو رفض تنفيذ — "
    ...
    lines.append(
        f"**تحسّن حقيقي لكن متواضع، يستاهل المتابعة بحذر لا اعتماد فوري**: الالتزام بهدف حقيقي (`maxTP` أو "
        f"2R كحد أدنى) بدل الإغلاق اليدوي رفع معامل الربح بنسبة {_fmt(hybrid_row['pf_improvement_pct'], 0)}% "
        "فقط (وليس 110%+ كما ذُكر بنسخة سابقة خاطئة) — صمد أمام اختبارات المتانة (مونت كارلو، ثبات سنوي، "
        "تحقق على بيانات معزولة) لكن بحجم تأثير صغير إحصائياً (Cohen's d صغير رغم دلالة p). **الخطوة التالية "
        "المقترحة**: تجربة ورقية (paper trading) محدودة لقاعدة بسيطة — وضع أمر جني ربح معلَّق عند الهدف "
        "الحقيقي المسجَّل فور الدخول (للصفقات التي لها `maxTP` معروف تحديداً؛ الـ2R الافتراضي لبقية الصفقات "
        "تخمين وليس قاعدة مؤكَّدة) — قبل أي اعتماد على رأس مال حقيقي. فرضية \"التخصيص حسب نمط الدخول\" لم "
        "تُثبت قيمة مضافة هنا ولا داعي لتعقيد التنفيذ بها.\n"
    )
